...
async function copy({
  ...
  while (true) {
    const where = []
    if (lastTime) where.push(`time > '${lastTime.toNanoISOString()}'`)
    else if (beginsAt) where.push(`time >= '${beginsAt.toISOString()}'`)
    if (endsBefore) where.push(`time < '${endsBefore.toISOString()}'`)

    const query = `SELECT * FROM "${mmName}" ${
      where.length ? 'WHERE' : ''
    } ${where.join(' AND ')} LIMIT ${queryLimit}`
